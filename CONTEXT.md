...
artifact of the near-complete scored graph rather than a defect in module 14 (R-26).
**Module 16, the Visualization API, is built as of 2026-09-20** (ADR-0080 through ADR-0087),
**and the L9 orchestration layer with it** — the larger of the two, because
`causalog.orchestration` was an empty scaffold and the pipeline existed only inside a script
that printed `[4/9]` and returned `None` when anything refused. The pipeline is now DATA:
fifteen declared stages, twelve runnable, and **modules 7, 8 and 15 declared `NOT_RUNNABLE`
with the module named rather than omitted** — an omitted stage is indistinguishable from one
that ran and found nothing. It runs as a resumable job with failure isolation over an
append-only stage ledger (migration 0017). **Every response carries its `OutputEnvelope`, its
unflattened provenance, its evidence references and its §55 timing, and no endpoint can return
a confidence without its components** — enforced by the type and by a blocking law test over
the live route table AND the AST of `api/routes/`, both observed to REJECT a planted
violation. `api_schema_version` 1.0.0 is **FROZEN for the shell only**; payload bodies stay
`draft` under modules that in three cases do not exist, because freezing those would be
OQ-009's error in a new place. **The determinism gate runs for the first time** — two runs
over a materialized dataset are byte-identical across nine artifacts by the gate's own
`compare()` — but no clean layer exists in CI, so OQ-014 stays open and `continue-on-error`
stays. **Five defects were found and fixed**, four of them invisible until something finally
assigned an adapter to a port (§11). Three of the 16 modules remain.
...
| HTTP API **shell** (prd.md §53) | Visualization API | Frontend | **frozen** at `api_schema_version` 1.0.0 | **ADR-0085**, specified in `docs/api.md` and generated into `docs/openapi.json`. The freeze covers the ROUTE SET, `ApiResponse`/`ResponseScope`, the error taxonomy and its status mapping, pagination, the auth and role contract, idempotency, and the job/stage shapes. It does **not** cover payload bodies: every reasoning artifact this API serializes is still `draft` and three of their owning modules (7, 8, 15) do not exist, so freezing them would be the assertion-not-specification error OQ-009 exists to prevent, in a new place. Bodies are versioned by their owning modules |
...
| `ApiResponse`, `ResponseScope` | `api/schemas/envelope.py` | Frontend | **frozen** with the shell | **ADR-0080** — generic over its payload; `envelope` is required under `RUN_SCOPED` and refused under `CATALOG`. `confidence` is a `ConfidenceView` whose `components` carries `min_length=1`, so the API cannot return a bare float wearing a confidence's name. Enforced over the live route table AND the AST of `api/routes/`, both observed to reject a planted violation |
| `ApiError`, `ERROR_STATUS`, `PUBLIC_MESSAGES` | `api/schemas/errors.py` | Frontend | **frozen** with the shell | **ADR-0085** — total over the closed `CausaLogError` taxonomy; an undeclared member raises rather than defaulting to 500. Outside production the engine's own message is returned; in production it is REPLACED wholesale by a fixed sentence per member, never filtered |
| `Role`, `Capability`, `PERMISSIONS` | `core/ports/identity.py`, `api/security/` | Visualization API | **frozen** with the shell | **ADR-0081** — exactly prd.md §10's five user types under domain-neutral names, because `api` is now in the LAW-DOMAIN scan. The matrix is DATA, keyed by capability rather than by route path, and is rendered into `docs/api.md` from the value rather than retyped |
| `Principal`, `Authenticator` | `core/ports/identity.py` | Visualization API | draft | **ADR-0082** — the port is the seam; V1's adapter verifies a stdlib HMAC signed token and adds no dependency. Signature covers the TRANSPORTED bytes, comparison is `compare_digest`, expiry is checked against the injected `Clock`. No key rotation and no revocation — see the ADR's negative consequences |
| `JobStore`, `JobRecord`, `JobStatus`, `StageStatus`, `StageTransition` | `core/ports/jobs.py` | Orchestration (wiring); Visualization API | draft | **ADR-0083** — the execution ledger. `pipeline_stage` holds TRANSITIONS and is append-only at the database, so a retry cannot overwrite the failure it retried. `PARTIAL` is not a synonym for `FAILED`, and `NOT_RUNNABLE` is neither |
| `RateLimiter`, `RateLimitVerdict` | `core/ports/limits.py` | Visualization API | draft | **ADR-0084** — a SEPARATE port from `DerivedCache` though Redis serves both: the cache may be flushed without changing an answer, limiter state may not. Fails OPEN, which is why authorization is not built on it |
| `Stage`, `PIPELINE_STAGES`, `PipelineRequest`, `PipelineState` | `orchestration/stages.py` | Visualization API; the determinism gate | draft | **ADR-0083** — the pipeline as DATA. Fifteen stages declared, twelve runnable; modules 7, 8 and 15 are DECLARED `NOT_RUNNABLE` with the module named, never omitted, because an omitted stage is indistinguishable from one that ran and found nothing |
| `EngineFacade`, `QueryResult`, `RunWorkspace` | `orchestration/facade.py` | Visualization API **only** | draft | The one surface L10 may call (F5). Every method returns its payload WITH the envelope, provenance, evidence and timing, so a route cannot assemble a response that omits them |
| `orchestration.views.*` | `orchestration/views.py` | Visualization API | **frozen** with the shell (hand-written views only) | The F5 translation seam: `api` may import only `core`, `orchestration` and itself, so it cannot name a reasoning type. Hand-written views carry what the API GUARANTEES; everything else travels as `canonical_form` output, which is the frozen wire shape (ADR-0023) and is marked `draft` per endpoint in `docs/api.md` |
...
| `postgres_schema_version` | `1.1.0` | `deployment/sql/migrations/` | Set 2026-08-29 by ADR-0032/ADR-0033. Fifteen paired migrations; the authority on what is applied is the `schema_migration` ledger, not this row. Does NOT participate in `run_id`: the schema decides how a fact is stored, never what it is. | **Moved to 1.1.0 on 2026-09-20 by ADR-0083**, which adds migration 0017: `pipeline_job`, the append-only `pipeline_stage` ledger, and `idempotency_record`. Seventeen paired migrations.
| `api_schema_version` | `1.0.0` | `causalog.api` | **Set and FROZEN 2026-09-20 by ADR-0085.** semver. Versions the API **shell** — routes, `ApiResponse`, the error taxonomy, pagination, the auth and role contract, idempotency, and the job/stage shapes — and NOT the payload bodies, which stay `draft` under their owning modules. Does NOT participate in `run_id`: the API describes a run, it is not an input to one. Published at `/healthz` and generated into `docs/openapi.json`, which `make laws` and CI check for drift. |
...
| OQ-014 | The determinism gate could not run: `scripts/check_determinism.py` exited 2 `NOT-YET-RUNNABLE` because no orchestration pipeline entry point existed, and its CI job is `continue-on-error` for that reason alone. Every `RunKey` input has had a real value since ADR-0044. **Substantially unblocked 2026-09-20 (ADR-0083): the entry point now exists** at `causalog.orchestration.pipeline`, and the gate RUNS rather than exiting 2. It was exercised twice over a materialized dataset and the gate's own `compare()` reported **0 mismatches across 9 artifacts** (envelope, events, entities, timelines, states, transitions, causal graph, confidence report, stage report) — evidence in `PROGRESS.md`. **It is NOT closed**, for one honest reason: the DataCo source file is not in the repository, so no clean layer exists here or in CI, and the gate reports `NOT-RUNNABLE` rather than passing. The entry point refuses to exit 0 on an unmaterialized dataset precisely so that two identically-failing runs cannot be scored as byte-identical. | Keep the script reporting the gap loudly. Remove `continue-on-error` when CI can materialize a dataset, not before — removing it now would make a job that cannot run look like one that passes, which is the failure this question is about. | **Medium**, reduced from the original. The gate machinery is now demonstrated to work; what remains is a dataset in CI. | A materialized dataset in CI |
...
| ~~OQ-027~~ **RESOLVED 2026-09-20 by ADR-0087** | **A plateau is reported, and nothing says what a consumer should DO with one.** 310 scored links sit at exactly 0.400000, so under `weakest_link_v1` many chains compose identically and the recommended set is frequently unsequenceable. | `RootCauseView.recommended_groups` is a tuple of `PlateauGroupView`, not of causes. A group carries `tied` and, when tied, a required `plateau_notice`; members are sequenced by `event_id` only so the body is byte-identical across runs, and that sequence carries no ranking. The tie is NOT broken on earliness. | **Medium.** An interface that renders a tie as a ranked list makes the engine appear to have chosen, which is the one thing the plateau notice exists to deny. | ADR-0087 (accepted) |
...
| ~~OQ-031~~ **RESOLVED 2026-09-20 by ADR-0087** | **prd.md §51 Workspace 5 asks for no simulated-versus-observed distinction**, while §37 requires that distinction "in the implementation or the user interface". | §37 governs; §51 is underspecified. `SimulatedEventView` is a separate wire type from `EventView`, with no `occurred_at` and a `provenance_class` constrained to `SIMULATED`, so a client cannot deserialize one into the other and no interface can render them identically by accident. | **Medium.** A workspace built to §51 as written would render a simulated figure exactly like an observed one. | ADR-0087 (accepted) |
...
| OQ-033 | **prd.md names no administrator, yet three capabilities need a holder.** §10's five user types are all consumers of analysis; none is described as administering the system. `MUTATE_DATASET`, `EXECUTE_PIPELINE` and `DELETE_INFERRED` must nevertheless be held by somebody. | Default: `DATA_SCIENTIST` holds all three and `PROCESS_IMPROVEMENT` also holds `EXECUTE_PIPELINE` (ADR-0081). Pinned by `tests/api/test_permission_matrix.py` so a change is deliberate. This is a proposed default, not a ruling. | **Medium.** The default hands a data scientist the ability to delete a run's inferences, which is a product decision the API layer is not entitled to make. | A product owner's ruling |
| OQ-034 | **Stage wiring now exists twice.** `causalog.orchestration.stages` declares the pipeline (ADR-0083) and `scripts/build_causal_graph.py` still holds the nine-stage function its three sibling scripts import. Two copies of one sequence will drift, and the first symptom is two reports describing one slice while disagreeing about it — the DEF-0004 shape this repository keeps recording. | Re-point the four scripts at `orchestration.stages` and delete `run_pipeline`. Not done in the module 16 commit because it would mix a refactor with a behaviour change, which `CONVENTIONS.md` §13 forbids in one commit. | **Medium.** Both paths are exercised — the scripts write the committed reports, the orchestration path serves the API — so a divergence is a real disagreement rather than dead code. | A follow-up commit |
| OQ-035 | **`empty_because` is prose, so a client cannot branch on WHY a result is empty.** ADR-0087 makes an empty result state its reason, which is a large improvement over an unexplained `[]`, but the reason is an English sentence: neither machine-readable nor translatable. | Add a structured reason code beside the prose (`ALL_CLAIMS_REFUSED` / `NOTHING_PROPOSED` / `SEED_ISOLATED`), keeping the sentence for humans. Deferred rather than guessed at, since the useful set of codes is not yet known from real client use. | **Low.** A client can already branch on `rejected_claim_count`; the prose is a convenience that will eventually be parsed by somebody. | Module 16's first real consumer |
...
2026-09-20  Module 16 (Visualization API) and the L9 orchestration layer. ADR-0080..0087.
            The engine can be ASKED something for the first time: `causalog.orchestration`
            and `causalog.api` were both empty scaffolds, and the pipeline existed only
            inside a script that printed [4/9] and returned None when anything refused.
            The pipeline is now DATA -- 15 declared stages, 12 runnable, and modules 7, 8
            and 15 DECLARED NOT_RUNNABLE with the module named rather than omitted -- run
            as a resumable job with failure isolation (a refusal blocks its dependents and
            spares independents; the execution ends PARTIAL, which is not a synonym for
            FAILED) over an append-only stage ledger (migration 0017, postgres_schema_version
            1.0.0 -> 1.1.0). Every response carries its OutputEnvelope, its provenance
            summary with the five classes unflattened, its evidence references and its
            §55 timing, and NO endpoint can return a confidence without components --
            enforced by the type and by a blocking law test over the live route table AND
            the AST of api/routes/, both observed to REJECT a planted violation. Roles are
            prd.md §10's five user types under domain-neutral names and the permission
            matrix is DATA, tested over every role x every route; `api` and `orchestration`
            join the LAW-DOMAIN scan, which caught three violations in this change's own
            prose on its first run. api_schema_version 1.0.0 is FROZEN for the SHELL only
            -- routes, envelope, errors, pagination, auth, idempotency, job shapes -- and
            NOT for payload bodies, which stay draft under modules that in three cases do
            not exist; freezing those would be OQ-009's error in a new place.
            docs/api.md is the contract and docs/openapi.json is generated from the
            application and drift-checked in make laws and CI. OQ-027 and OQ-031 close
            (a plateau is an unsequenced SET; a simulation is a separate TYPE with no
            occurred_at). OQ-014 remains OPEN, though substantially unblocked: its entry point
            now exists, the gate RUNS instead of exiting 2, and two runs over a materialized
            dataset are byte-identical across 9 artifacts by the gate's own compare() --
            but no clean layer exists in CI, so the gate reports NOT-RUNNABLE and
            continue-on-error stays. OQ-033..OQ-035 opened. FIVE defects found and fixed,
            four of them invisible until something finally assigned an adapter to a port:
            AUDITABLE_ACTIONS enforced by one implementation and not the fake;
            FactRepository's declared signatures matching neither adapter; two correlation
